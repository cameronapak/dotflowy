...
*effect* — async I/O, error handling, retry/timeout, resource lifecycle, concurrency,
...
the client *and* the Worker, and zod is removed from the project entirely.** `src/data/schema.ts` (the
...
default, not "Effect leaks into every runtime." Three seams stay non-Effect-*shaped* at the surface, but
...
- **Effect underneath, throw at the seam — the TanStack mutation handlers**
  ...
  because TanStack DB signals failure by *throwing* (a throw triggers optimistic rollback). They run an
