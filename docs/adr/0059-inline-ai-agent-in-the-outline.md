...
| Tools            | Read + additive only (`get_outline`, `search_nodes`, `export_opml`, `add_node`, `add_subtree`, `add_to_today`, `mirror_node`, `mirror_to_today`) plus allowlisted **`web_search`** (Firecrawl now via `FIRECRAWL_API_KEY`; CF AI Gateway / Exa later). Key unset → tool omitted (fail closed). Citations = inline markdown links `[title](url)` in the answer body (links plugin folds them). Denied: `update_node`, `delete_node`, `move_nodes`, `import_opml`. Engine re-fire replace is not a model tool. |
| Honesty          | System prompt carries a short canonical product brief (outline app; **not** Notion sync; upgraded sync experimental; Dot = paid inline beta; MCP = external agents). Dot may answer "what is Dotflowy?" but must **look it up** via `web_search` / tools — never invent integrations or capabilities; say unsure if search fails.                                                                                                                                                                            |
...
4. **Atom focus on `@dot` chrome** (bible-chip pattern): Arrow select →
   Enter/Space opens Run confirm; Delete removes the token — wire into both
   row and zoomed-title keymaps (two-path trap).
5. **Swap/augment `web_search` backend** (CF AI Gateway web search, Exa, …)
   behind the same tool name + citation contract when Firecrawl is not the
   preferred prod path.
