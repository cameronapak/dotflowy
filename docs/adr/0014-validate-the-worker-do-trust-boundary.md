...
2. **Atomicity (Cloudflare-native, DO-side — deliberately NOT Effect).** Each DO node mutation
   ...
   *out* of the SQL step (`commitChange` split into `recordChange` + `broadcastChange`) so a frame is
...
Validation removes the *common* cause of a mid-loop throw (bad input); the transaction covers the
*residual* (a genuine SQLite fault on valid input). Together: a bad body never reaches the DO, and if a
...
failure slots into the *existing* typed-error channel with no new machinery — `decodeUnknownEffect`
...
SQLite loop would be heavyweight and buy nothing CF doesn't already give: `transactionSync` *is* the
...
intervening `await` are submitted atomically" — is about *durability batching*, and the docs are silent
...
- **Decode inside the DO methods, not at the Worker boundary.** Rejected: the DO should stay *total*
...
- **Zod (then the client's schema library) instead of Effect Schema.** Rejected: zod wasn't in the
  ...
  Schema and zod was removed from the project entirely — see [ADR 0003](./0003-no-zod-defaults-in-the-schema.md).
  Effect Schema is now the one schema language across client and Worker.)
