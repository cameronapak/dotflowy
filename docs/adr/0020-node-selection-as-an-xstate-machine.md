...
**currently implicit and the cost of getting it wrong is real**, _and_ where converting doesn't
...
is the textbook _shape_ but the wrong _target_: it is already a clean, tested Effect resource, so an
...
rest. This is the honest scope of the "XState + Effect Schema" combo _for this feature_ — for the
...
advertises _per-state context narrowing_ — a field that exists only in one state's type — which would
...
in _every_ state regardless of the per-state schema. A typed probe confirmed it: a `@ts-expect-error`
...
`selecting` carries the run. This isn't a fallback — it's the _correct_ shape here: `SelectionData |
...
compile-time guarantee. The safety that _is_ robust comes from **event-level** narrowing: `extend` and
...
subscription: it runs the selector on every snapshot but re-renders a row only when _its_ edge
