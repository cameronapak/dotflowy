...
device arrives as a `{type:'change', seq, ops}` delta — no window-focus refetch. Writes still
POST/PATCH/DELETE `/api/nodes` optimistically; the socket echo reconciles idempotently. Reconnect
