...
export function parseMarkdownForest(
  ...
  for (const line of lines) {
    ...
    if (heading) {
      ...
      while (headings.length && headings[headings.length - 1]! >= level) headings.pop()
...
function pushListLine(
  ...
  if (!indents.length || indents[indents.length - 1]! < width) indents.push(width)
...
export function planMarkdownPaste(args: {
  ...
  const asChildren = placement === 'child-prepend' ? [...first.children, ...rest] : first.children
  ...
  if (childIds.length && existingFirstChild) {
    repoints.push({ id: existingFirstChild.id, prevSiblingId: childIds[childIds.length - 1]! })
  }

  const siblings = childrenOf(index, anchor.parentId)
  const anchorAt = siblings.findIndex((n) => n.id === anchorId)
  const existingNext = anchorAt !== -1 ? (siblings[anchorAt + 1] ?? null) : null
  const siblingIds = emit(asSiblings, anchor.parentId, anchorId)
  if (siblingIds.length && existingNext) {
    repoints.push({ id: existingNext.id, prevSiblingId: siblingIds[siblingIds.length - 1]! })
  }
