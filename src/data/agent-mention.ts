 ...
 * `@dot` and `@agent` are interchangeable names for the ask target.
 */
...
/** Tag-only helper for `/ask` — does not fire an ask. */
export function ensureAgentMention(
  nodeId: string,
  text: string,
  onTextChange: (id: string, text: string) => void,
): void {
  if (hasAgentMention(text)) return;
  const next = text.trim().length ? `@agent ${text}` : "@agent ";
