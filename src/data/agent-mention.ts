...
/**
 * Literal `@agent` mention (ADR 0059). Same boundary rules as `#tag`: start or
 * whitespace before, word-boundary after so `@agency` stays literal.
 */
export const AGENT_MENTION = "(?<=^|\\s)@agent(?=$|\\s|[^\\p{L}\\p{N}_-])";

const AGENT_RE = new RegExp(AGENT_MENTION, "gu");

const EMPTY: string[] = [];

/** Distinct `@agent` tokens in `text`, first-seen order. */
export function parseAgentMentions(text: string): string[] {
  if (!text.includes("@agent")) return EMPTY;
  const out: string[] = [];
 ...
 * mentioned `@agent` on this line, or the node carries agent `origin` (answer
 * children). Distinct from provenance's `is:agent` (origin-only).
 */
