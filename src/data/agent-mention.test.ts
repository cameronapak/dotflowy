...
describe("ensureAgentMention", () => {
  test("prefixes @agent when missing", () => {
    let text = "hello";
    ensureAgentMention("n1", text, (_id, next) => {
      text = next;
    });
    expect(text).toBe("@agent hello");
  });

  test("no-op when already mentioned", () => {
    let calls = 0;
    ensureAgentMention("n1", "@dot hi", () => {
      calls += 1;
    });
    expect(calls).toBe(0);
  });
