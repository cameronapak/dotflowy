...
function roundTrip(forest: Shape[], mirrorOf: Record<string, string> = {}): Shape[] {
...
describe('parse(outlineToMarkdown(t)) === t', () => {
  ...
  test('holds for a task with no text', () => {
    expect(roundTrip([shape('', [], true, false)])).toEqual([shape('', [], true, false)])
    expect(roundTrip([shape('', [], true, true)])).toEqual([shape('', [], true, true)])
  })

  test('exception 3: a mirror flattens to an independent copy', () => {
    // n0 "source" > n1 "kid"; n2 mirrors n0.
    const { index } = buildIndex(
      [shape('source', [shape('kid')]), shape('')],
      { n2: 'n0' },
    )
  ...
  test('exception 3: a mirror inside its own source emits once and stops', () => {
    ...
    const { index } = buildIndex([shape('source', [shape('snapshot')])], { n1: 'n0' })
    expect(outlineToMarkdown(index, ['n0'])).toBe(['- source', '  - source'].join('\n'))
  })

  test('exception 3: one source mirrored into two branches expands in both', () => {
    const { index } = buildIndex(
      [shape('src', [shape('kid')]), shape('a', [shape('')]), shape('b', [shape('')])],
      { n3: 'n0', n5: 'n0' },
    )
...
describe('parseMarkdownForest', () => {
  ...
  test('a skipped heading level clamps instead of jumping', () => {
    ...
    expect(forestShape(forest)).toEqual([shape('A', [shape('E', [shape('text')])])])
  })
  ...
  test('list indentation nests inside the heading floor', () => {
    const forest = parseMarkdownForest('# A\n- one\n  - two')
    expect(forestShape(forest)).toEqual([shape('A', [shape('one', [shape('two')])])])
  })
  ...
  test('task markers map to isTask/completed', () => {
    expect(forestShape(parseMarkdownForest('- [ ] open\n- [x] done\n- [X] DONE'))).toEqual([
  ...
  test('fences suppress the grammar and keep their delimiters', () => {
    const forest = parseMarkdownForest('```ts\n- not a bullet\n  indented\n\n```\nafter')
    expect(forestShape(forest)).toEqual([
      shape('```ts'),
      shape('- not a bullet'),
      shape('  indented'),
  ...
  test('literal mode keeps a diff pasteable', () => {
    expect(parseMarkdownForest('- old\n+ new', { literal: true }).map((n) => n.text)).toEqual([
      '- old',
      '+ new',
    ])
  })
...
describe('planMarkdownPaste', () => {
  ...
  let n = 0
  const plan = (md: string, head = '', tail = '', placement: 'sibling' | 'child-prepend' = 'sibling') => {
    n = 0
  ...
  test('the anchor absorbs the first line; its children prepend', () => {
    ...
    expect(p.inserts).toEqual([
      { id: 'p0', parentId: 'A', prevSiblingId: null, text: 'two', isTask: false, completed: false },
      { id: 'p1', parentId: null, prevSiblingId: 'A', text: 'three', isTask: false, completed: false },
    ])
  ...
  test('a plain first line never un-tasks the anchor', () => {
    expect(plan('plain\nb')!.anchor).toEqual({ text: 'plain', isTask: null, completed: null })
  })
  ...
  test('inserts are depth-first pre-order with the sibling chain wired', () => {
    const p = plan('one\nA\n  A1\n  A2\nB')!
    expect(p.inserts.map((i) => `${i.text}:${i.parentId}:${i.prevSiblingId}`)).toEqual([
      'A:null:A',
      'A1:p0:null',
      'A2:p0:p1',
      'B:null:p0',
