...
describe("individual patterns", () => {
  ...
  test("underscore italic is INTRAWORD-guarded (snake_case stays literal)", () => {
    expect(matchesAnywhere(ITALIC_UNDERSCORE_PATTERN, "snake_case_here")).toBe(
      false,
    );
    expect(matchesAnywhere(ITALIC_UNDERSCORE_PATTERN, "a1_b_c2")).toBe(false);
    expect(matchesAnywhere(ITALIC_UNDERSCORE_PATTERN, "word_italic_")).toBe(
      false,
    );
  });
});
...
describe("prefix disambiguation (lower precedence wins on overlap)", () => {
  test("the combined alternation picks bold before italic on a ** run", () => {
    ...
    const combined = new RegExp(`^(?:${BOLD_PATTERN}|${ITALIC_PATTERN})$`);
