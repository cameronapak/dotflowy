    ...
    withHarness(
      ...
      ({ sockets }) =>
        Effect.gen(function* () {
          ...
          yield* settle
          expect(helloOf(nth(sockets, 0))).toEqual({ type: 'hello', since: 42 })
        }),
    ))
    ...
    withHarness(
      ...
      ({ sockets }, resync) =>
        Effect.gen(function* () {
          ...
          yield* settle
          expect(helloOf(nth(sockets, 0))).toEqual({ type: 'hello', since: 99 })
          ...
          expect(helloOf(nth(sockets, 1))).toEqual({ type: 'hello', since: null })
        }),
    ))
    ...
    withHarness(
      ...
      ({ sockets, collected }) =>
        Effect.gen(function* () {
          ...
          yield* settle
          const initialErrors = collected.filter((e) => e._tag === 'InitialError')
