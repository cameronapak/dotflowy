...
// can restore focus after the navigation. React Router types `location.state`
// as `any`, so this is read defensively as `{ pivotId?: string }` at the use
// site (no module augmentation like TanStack Router's `@tanstack/history`).
...
export function OutlineEditor({ rootId }: OutlineEditorProps) {
  ...
  const filterQuery = searchParams.get("q") ?? undefined;
  // Stable object until the `?q=` string actually changes -- keeps viewCtx (and
  // therefore every memoized OutlineNode) from re-rendering on each keystroke.
  const routeSearch = useMemo<{ q?: string }>(
    () => ({ q: filterQuery }),
    [filterQuery],
  );
...
/**
 * First-run bootstrap: import a pre-Wasp localStorage outline if present, else
 * seed the welcome bullets. Both await the collection's initial server load and
 * no-op unless the user's silo is empty (seed.ts / import-legacy.ts), so this
 * is safe to call unconditionally on mount.
 *
 * bootstrapOutline returns a BootstrapError as a value (errore convention) when
 * the initial load failed — it detects that deliberately, because the query
...
function useZoomNavigation({
  ...
  const pivotId = (location.state as ZoomHistoryState | null)?.pivotId ?? null;
  ...
  const navigateZoom = useCallback(
    (toRootId: string | null, pivot: string) => {
      ...
      if (prefersReducedMotion()) {
        ...
        if (toRootId === null) navigate("/", { state });
        else navigate(`/${encodeURIComponent(toRootId)}`, { state });
        return;
      }
      ...
      const to =
        toRootId === null ? "/" : `/${encodeURIComponent(toRootId)}`;
      const state = { pivotId: pivot };
      navigateWithZoomTransition(navigate, to, state);
    },
    [focusIndex, refs],
  );
...
/** React Router's `viewTransition` flag has no `types` — wrap navigate in
 *  `startViewTransition({ types: ['zoom'] })` so styles.css's zoom morph rules
 *  activate (PRD Phase 3 view-transition preservation). */
