...
- Every mutator patch, delete, and get must pass `expectedTable`. An unscoped id lookup builds `UNION ALL` across the shard tables and hits the compound-SELECT limit in Workerd SQLite. The typed `MutationCtx` hides the `expectedTable` argument, and `ctx.db.asId(...)` is compile-time branding only, so an `asId`-wrapped write is STILL unscoped at runtime. Write through the per-table facade (`ctx.db.nodes.delete(...)`, `ctx.db.dailyIndex.patch(...)`), which forwards its table name as `expectedTable`. This shipped broken once, in `lunora/mcp.ts`, and took out every date-resolving MCP tool.
