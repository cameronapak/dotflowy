...
Layout:
- **`types.ts`** — the typed contract: `definePlugin`, the `El` descriptor, `TokenSpec`, `InteractionSpec`, `ViewTransform`, `InputSpec`, and `PluginContext` (D8: the promoted `NodeCommands` + tree reads + `nav` + `openOverlay`).
...
- **`registry.ts`** — derives the combined token regex + dispatch, the interaction dispatch (`blocksCaret`/`dispatchClick`/`dispatchContextMenu`), the view-transform composition (`composeHidden`/`buildViewFilter`), and the paste chain (`pasteReplacement`) from the array, once at load. The core (`inline-code.ts`, `paste.ts`, `OutlineEditor`, `tree-store.ts`) consumes these and stays generic.
...
**Still core-wired (not yet plugin seams):** slash commands (C), per-bullet keymap (D), the checkbox row slot (F), and the `#`/`/` autocomplete menus (H). So **todos is only a partial plugin** (Seam G hide-completed; its checkbox/keymap/`/todo`/`[]` surface is still core in `OutlineNode`), and **tags' `#` menu (H) remains in the core** (`tag-menu.tsx`). Fade-inheritance still reads `node.completed` in `OutlineNode` (stays core until a row-decoration seam). Those need a menu engine + the todos interaction-surface move — the next implementation passes per ADR 0018.
