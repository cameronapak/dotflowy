...
- **Every button `preventDefault`s on `onPointerDown`** to keep the caret/keyboard alive across the tap. **Dismiss is the lone exception** — it also `preventDefault`s but then calls `el.blur()` (dismissing the keyboard IS a blur).
...
- **Not e2e-testable → manual iPhone checklist in the PR:** keyboard positioning, `visualViewport` tracking, iOS focus-preservation under `preventDefault`. `e2e/mobile-actions-bar.spec.ts` covers coarse-only mount, focus/blur visibility, and each button's wiring.
