...
- **The DO routing key is separate from the owner.** `resolveUserId()` (in `worker/index.ts`) picks the caller's Durable Object — a **constant** today (single-user), a stable user id once real accounts land. **Never key the DO off the email/owner:** a DO name is permanent, so that would orphan a user's whole outline on any email/auth change.
...
- **Dev loop:** run `bun run dev` (Vite) *and* `bun run dev:api` (`wrangler dev` on :8787, Worker + the DO + local D1); first time `bun run db:migrate:local` (for the legacy/import D1). `bun run cf:dev` is a production-like single-server preview.
- **Migrations:** the SQL files in `migrations/` (`bun run db:migrate:local` / `:remote`, run `:remote` **before** the first `bun run deploy`) are **D1** migrations. The DO's own schema is created in its constructor (no SQL file); it's registered via the `new_sqlite_classes` tag in `wrangler.jsonc`.
...
- **Nodes live in a per-user Durable Object's SQLite** ([per-user DO sync](./docs/DECISIONS.md#sync-via-a-per-user-durable-object)). `nodesCollection` is a TanStack DB query collection over `/api/nodes` (`collection.ts` + `api.ts` + `query-client.ts`); the contract is unchanged, so store/mutations/components didn't change — the Worker just routes `/api/*` to the caller's DO instead of querying D1. **Side-collections (`tag-colors.ts`, `daily-index.ts`) ride the same DO** over a generic `/api/kv?collection=<name>` store (`kv-api.ts`); each passes its **concrete** zod schema inline (a generic factory loses schema inference). The old `dotflowy-oss:*` localStorage keys are no longer read.
- **First-run bootstrap = import-or-seed.** On mount `OutlineEditor` calls `bootstrapOutline()` (`seed.ts`): `importLegacyNodes()` first (one-time pre-sync localStorage → the synced store via the API, guarded by a `dotflowy-oss:d1-imported` flag, non-destructive), then `seedIfEmpty()` only if nothing imported. Keep the **single guard in `bootstrapOutline`** — splitting it races import vs. seed under StrictMode. Covered by `e2e/import-legacy.spec.ts`. **Separately, server-side,** the Worker does a one-time non-destructive copy of any pre-DO **D1** rows into the user's DO on first read (`ensureSeeded`); the DO marks itself `seeded` and never re-imports.
