...
An opt-in view mode ([ADR 0033](./docs/adr/0033-spotlight-focus-mode.md)) that dims the outline to `opacity: 0.3` except the **active branch** — the focused bullet plus its ancestor chain up to the zoom root, which stay full. Toggled from the header More menu ("Spotlight mode"), **default OFF**, persisted per-browser in `localStorage` (`dotflowy:spotlight`) — a view preference, **not** synced data and **not** a `dotflowy:flag:*` dev flag. The toggle store (`src/data/spotlight.ts`) mirrors `show-completed-provider` (`useSyncExternalStore`); `useSpotlightEnabled()`/`setSpotlightEnabled` live in `spotlight-mode.tsx` and `<SpotlightController>` (mounted once in `__root.tsx`) installs/tears down the engine on toggle.

- **Painted by ONE generated `<style>` keyed on `data-node-id`** — the `TagColorStyles` mechanism (ADR 0007), **not** React state. Focus changes on every caret move, so per-row React dimming would re-fight the ADR 0014 render budget; and the flat windowed list (ADR 0019) has no DOM nesting, so "active + ancestors" can't be a CSS descendant selector — the lit id set is walked from the `TreeIndex` parent chain (`getViewRootId()` is the stop) and emitted as an explicit list. A rule keyed on the id matches a row the instant it scrolls into the window.
- **`spotlight-on` on `<body>` means "actively dimming a focused row"**, not "mode enabled" — a document `focusin`/`focusout` listener (rAF-deferred blur, the `MobileActionsBar` `useOutlineEditing` pattern) adds it when an outline row is focused and **removes it entirely when nothing is** (so the resting state is full opacity, and node multi-selection — which has no caret — needs no special-casing). Only `.outline-row` dims; the header and the zoomed `ZoomedTitle` are never `.outline-row`, so they stay full.
- **Transition is input-modality-aware:** a global capture `keydown`/`pointerdown` sets `spotlight-fade` only when the last input was a pointer, so a click eases (~120ms) but keyboard arrow-nav snaps instantly; `prefers-reduced-motion` forces instant. `e2e/spotlight.spec.ts` covers the lit set, caret-follow, blur reset, and mode-off.
