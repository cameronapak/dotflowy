...
- **Reveal reflow lives in a `selectionchange` watcher** (`watchCaretReveal`), added in `onFocus` and torn down in `onBlur` (OutlineNode + ZoomedTitle) so it's live only while the bullet is focused; it bails on a link-free line and is suspended during IME composition. `onFocus` also reveals the link under the caret right away; `onBlur` folds everything (`decorate(el, readSource(el), null, false)`). Both still early-return for `hasLink` so the common (link-free) case is untouched. Links are tokenized **first** so their interior is opaque (a `#tag` inside a label never chips).
