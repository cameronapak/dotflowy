...
`==text==` — the fifth folding token ([ADR 0035](./docs/adr/0035-highlights-color-in-text.md)), the emphasis model verbatim: folds to one atomic `<mark data-src>`, reveals `==` fences as real `.md-punct` walk-through text. **No `Node` field, no migration.**
