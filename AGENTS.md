...
Layout:
- **`types.ts`** — the typed contract: `definePlugin`, the `El` descriptor, `TokenSpec`, `InteractionSpec`, `CommandSpec` (Seam C), `KeymapSpec` (Seam D), `SlotSpec` (Seam F), `ViewTransform`, `MenuSpec`, `InputSpec` (paste + `autoformat`), and `PluginContext` (D8: the promoted `NodeCommands` + tree reads + `nav` + `openOverlay`).
...
- Covered by `e2e/daily-notes.spec.ts` (creation+zoom, idempotency, protection, the `/` send-to-today command).
