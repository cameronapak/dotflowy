...
- **`index.ts`** — the one explicit ordered array `plugins = [todos, provenance, code, links, nodeLinks, routeBible, tags, emphasis, highlight, daily, childCount]`. Add a plugin = add a folder + one line. Array order is the precedence tiebreak and dispatch order.
...
| ---- | ---------- | ------ |
| **A** inline token | regex fragment + `render → El \| WidgetEl`, composed into one `gu` regex; core owns escaping. Folding token emits a `data-src` atom (`contenteditable="false"`); React mode mounts a `<dotflowy-widget>` TSX atom. Precedence: links 0 < node-links 5 < code 10 < route-bible 15 < tags 20 < emphasis 30-34 < highlight 35. | code, links, node-links, tags, route-bible, emphasis, highlight |
| **B** delegated interaction | one set of content-container handlers, dispatched by `target.closest(selector)`; core has zero feature knowledge. | links, node-links, tags, route-bible, highlight (right-click recolor) |
| **C** `/` command | `CommandSpec`; the `/` list is `[...commandSpecs, ...CORE]`. `/move` stays core. | todos (`/todo`,`/bullet`), daily ("Send to Today"), emphasis (`/bold` etc.), highlight (`/highlight`) |
...
- **Creation:** Seam C (`/bold` etc.) + Seam D (`Mod+B`/`Mod+I`/`Mod+U`/`Mod+Shift+X`, all browser-native contentEditable commands the keymap's preventDefault overrides) via the shared `src/components/wrap.ts` (source-space wrap-or-insert; promoted out of the emphasis plugin when highlight became its second consumer). Styles its tags with Tailwind utilities on the token `El` (the `util` field) — there is no plugin `styles` seam (ADR 0031 retired raw plugin CSS). Search/display flatten via `flattenInline` (`src/data/inline-text.ts` = `stripEmphasis ∘ stripHighlights ∘ stripLinks`). Covered by `e2e/emphasis.spec.ts`.
