...
The editor is a clean core extended by **plugins** — modules compiled into the bundle (an internal registry, not runtime-loaded), one per `src/plugins/<name>/`. `code`, `links`, and `tags` are themselves plugins (dogfooded), so the core carries no feature-specific branches for them. Why and the full seam design: [ADR 0018](./docs/adr/0018-plugin-architecture.md).

Layout:
- **`types.ts`** — the typed contract: `definePlugin`, the `El` descriptor, `TokenSpec`, `InteractionSpec`, `InputSpec`, and `PluginContext` (D8: the promoted `NodeCommands` + tree reads + `nav` + `openOverlay`).
- **`index.ts`** — the **one explicit, ordered array** `plugins = [code, links, tags]`. Add a plugin = add a folder + one line here (no `import.meta.glob`). Array order is the precedence tiebreak and the dispatch order.
- **`registry.ts`** — derives the combined token regex + dispatch, the interaction dispatch (`blocksCaret`/`dispatchClick`/`dispatchContextMenu`), and the paste chain (`pasteReplacement`) from the array, once at load. The core (`inline-code.ts`, `paste.ts`, `OutlineEditor`) consumes these and stays generic.
...
**Still core-wired (not yet plugin seams):** slash commands (C), per-bullet keymap (D), the checkbox row slot (F), the render-time view-transforms (G — tag `?q=` filter and hide-completed), and the `#`/`/` autocomplete menus (H). So **todos is not yet a plugin**, and **tags' filter (G) + `#` menu (H) remain in the core** (`OutlineEditor`/`OutlineNode`/`tag-menu.tsx`). Those need the view-transform store refactor (D9) and a menu engine — the next implementation passes per ADR 0018.
