...
- **C — command (`/` palette).** A plugin contributes `commands`: a `CommandSpec` (`label`/`icon`/`keywords`/`available(node)`/`run(nodeId, ctx)`). The list driving the `/` menu is `[...commandSpecs, ...CORE]` (`registry.commandSpecs` then the core's). **todos** owns `/todo` + `/bullet`; `/move` stays a **core** command (structural, not a feature). v1 keeps the bespoke `useSlashMenu` engine — only its command *list* is registry-driven; folding the palette into the Seam-H menu engine is a later pass.
...
- Covered by `e2e/daily-notes.spec.ts` (creation+zoom, idempotency, protection).
