...
The app is a static SPA that talks to the Worker over `/api/*`, so both servers must run. After starting or restarting them, verify the actual Vite URL (it may move to the next free port) with BOTH checks: from an authenticated browser page, `fetch("/api/nodes")` returns 200 and `ws://<vite-host>/api/sync` opens and emits the initial snapshot. A terminal `curl` without cookies may correctly get 401, but it should still be a Worker-shaped response rather than a Vite fallback. A passing HTTP check is not enough — if `/api/sync` closes before the snapshot, the browser is still missing the Worker WebSocket path. The Vite dev proxy must support WebSocket upgrades for `/api` (object form with `ws: true`); after changing `vite.config.ts`, restart Vite because proxy config is loaded only at server start.

For local UI testing, prefer a real local account created through the invite-gated auth flow. If a task explicitly needs an auth bypass, keep it local-only: use `.dev.vars`/uncommitted scaffolding, call out the dirty files, and exclude bypass changes from any branch or PR unless the user explicitly asks to productize that bypass. Never relax the Worker `/api` session gate as a drive-by convenience.

Testing the OAuth-gated `/mcp` endpoint locally needs `BETTER_AUTH_URL` + `BETTER_AUTH_TRUSTED_ORIGINS` in `.dev.vars` (the `wrangler dev` custom-domain simulation rewrites both the issuer and the request `Origin` to the prod domain) — see `.dev.vars.example` and [ADR 0026](./docs/adr/0026-agent-native-mcp-server.md).
...
- **Dev loop:** copy `.dev.vars.example` → `.dev.vars` and set `BETTER_AUTH_SECRET` (the Worker fails closed without it); run `bun run dev:api` (`wrangler dev` on :8787, Worker + the DO + local D1) *and* `bun run dev` (Vite, usually :3000 but it may choose the next free port); first time `bun run db:migrate:local`. Open the Vite URL, not the Worker URL, for HMR. Verify both authenticated `GET /api/nodes` and the `/api/sync` WebSocket through Vite; the proxy must pass WS upgrades (`ws: true`) and Vite must be restarted after proxy edits. `bun run cf:dev` is a production-like single-server preview. In prod set the secret with `wrangler secret put BETTER_AUTH_SECRET`.
