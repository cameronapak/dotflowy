...
- Every mutator patch, delete, and get must pass `expectedTable`. An unscoped id lookup builds `UNION ALL` across the shard tables and hits the compound-SELECT limit in Workerd SQLite. `expectedTable` is an optional parameter, so nothing forces it, and `ctx.db.asId(...)` is compile-time branding only — an `asId`-wrapped write is STILL unscoped at runtime. Write through the per-table facade (`ctx.db.nodes.delete(...)`, `ctx.db.dailyIndex.patch(...)`), which forwards its table name as `expectedTable`. This shipped broken once, in `lunora/mcp.ts`, and took out every date-resolving MCP tool.
- Remigrate inserts missing rows only. It does not rewrite any `parentId`, so orphaned daily nodes need a reattach heal.
- `nodesCollection.toArray` is `[]` while the flag is on, and it never errors. Read live nodes through `getLiveNodes()`. A direct read builds an empty tree index, which fails `mirrorNode` and turns a captured undo point into delete-everything. oxlint bans that import in `src/plugins/**`, `src/components/**`, and `src/routes/**`, minus three files that write to it.
- `nodesCollection.toArrayWhenReady()` resolves instantly while the flag is on, so it gates nothing. Await `whenNodesSyncReady()` instead. It fires on both paths, and on the load-error path too, so it cannot hang.
- `await tx.isPersisted.promise` does not make the mutator's own rows readable. TanStack DB drops the optimistic overlay on server confirm, and the confirmed rows land one macrotask later. Subscribe and wait for the row. Reading straight through the await reports failure for a write that landed.
