...
Layout:
- **`types.ts`** — the typed contract: `definePlugin`, the `El` descriptor, `TokenSpec`, `InteractionSpec`, `ViewTransform`, `MenuSpec`, `InputSpec`, and `PluginContext` (D8: the promoted `NodeCommands` + tree reads + `nav` + `openOverlay`).
...
- **`registry.ts`** — derives the combined token regex + dispatch, the interaction dispatch (`blocksCaret`/`dispatchClick`/`dispatchContextMenu`), the view-transform composition (`composeHidden`/`buildViewFilter`), the menu list (`menuSpecs`), and the paste chain (`pasteReplacement`) from the array, once at load. The core (`inline-code.ts`, `paste.ts`, `menu-engine.tsx`, `OutlineEditor`, `tree-store.ts`) consumes these and stays generic.
...
- **I — input (paste).** The core owns the paste mechanics + plain-text baseline; each plugin's `input.onPaste` gets first say on the replacement string (links contributes the URL/anchor cases).
...
**Still core-wired (not yet plugin seams):** slash commands (C), per-bullet keymap (D), and the checkbox row slot (F). So **todos is only a partial plugin** (Seam G hide-completed; its checkbox/keymap/`/todo`/`[]` surface is still core in `OutlineNode`). The menu **engine** (H) exists, but the `/` palette still uses the core `useSlashMenu` (`slash-menu.tsx`) — it folds into the engine when its commands become a Seam-C registry. Fade-inheritance still reads `node.completed` in `OutlineNode` (stays core until a row-decoration seam). Those are the next implementation passes per ADR 0018.
