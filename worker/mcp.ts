...
const toolList = tools.map((tool) => {
  ...
  if (Object.keys(doc.definitions).length) inputSchema['$defs'] = doc.definitions
  return {
    name: tool.name,
...
const decodeOrNull = <S extends Schema.Top & { readonly DecodingServices: never }>(
  schema: S,
  value: unknown,
): S['Type'] | null => {
...
function handleToolCall(
  ...
  if (!call) {
    return Effect.succeed(rpcError(id, INVALID_PARAMS, 'expected params {name, arguments}'))
  }
  const tool = tools.find((t) => t.name === call.name)
  if (!tool) return Effect.succeed(rpcError(id, INVALID_PARAMS, `unknown tool: ${call.name}`))
  ...
  return decodeInput(call.arguments ?? {}).pipe(
    ...
    Effect.flatMap((input) =>
      tool.handle(input, store).pipe(
        ...
        Effect.catchTag('ToolError', (e) =>
          Effect.succeed(
            rpcResult(id, { content: [{ type: 'text', text: `Error: ${e.reason}` }], isError: true }),
          ),
        ),
      ),
    ),
    ...
    Effect.catchDefect(() => Effect.succeed(rpcError(id, INTERNAL_ERROR, 'internal error'))),
  )
}
...
export function handleMcp(request: Request, store: OutlineStore): Effect.Effect<Response> {
  ...
  return Effect.gen(function* () {
    ...
    if (Array.isArray(raw)) {
      ...
      return yield* Effect.fail(rpcError(null, INVALID_REQUEST, 'batch requests are not supported'))
    }
